...
pub struct ThreadView {
    ...
    /// Whether auto-approve mode is enabled for this thread.
    pub auto_approve_enabled: bool,
    /// Maximum number of auto-approvals (0 = unlimited).
    pub auto_approve_max_count: u32,
    /// Counter of auto-approvals performed.
    pub auto_approve_count: u32,
    /// Whether auto-cycle mode is enabled for this thread.
    pub auto_cycle_enabled: bool,
    /// The custom prompt to use for auto-cycle heartbeats.
    pub auto_cycle_prompt: SharedString,
    /// Interval in seconds between auto-cycle heartbeats.
    pub auto_cycle_interval_secs: u64,
    /// Task handle for the auto-cycle timer.
    pub auto_cycle_task: Option<Task<()>>,
    /// Whether auto-summary is enabled. When enabled and token usage reaches
    /// the threshold, automatically starts a new thread from a summary.
    pub auto_summary_enabled: bool,
    /// Token usage threshold (in tokens) for triggering auto-summary.
    /// Defaults to 75% of the model's max_tokens.
    pub auto_summary_threshold_tokens: u64,
    /// Whether the thread needs automatic summarization due to token limit.
    pub needs_auto_summary: bool,
    /// If set, the next Stopped event on this thread (from a new auto-summary thread)
    /// will auto-send this prompt.
    pub pending_continue_prompt: Option<SharedString>,
}
...
impl ThreadView {
    pub(crate) fn new(
        ...
        let mut this = Self {
            ...
            auto_cycle_task: None,
            auto_summary_enabled: false,
            auto_summary_threshold_tokens: u64::MAX,
            needs_auto_summary: false,
            pending_continue_prompt: None,
        };
    ...
    fn toggle_auto_summary(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        self.auto_summary_enabled = !self.auto_summary_enabled;
        if self.auto_summary_enabled && self.auto_summary_threshold_tokens == u64::MAX {
            // Set a default threshold: 75% of the model's max context.
            if let Some(usage) = self.thread.read(cx).token_usage() {
                self.auto_summary_threshold_tokens = (usage.max_tokens as f64 * 0.75) as u64;
            }
        }
        cx.notify();
    }

    fn toggle_auto_cycle(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        self.auto_cycle_enabled = !self.auto_cycle_enabled;
        if self.auto_cycle_enabled {
            self.start_auto_cycle(window, cx);
        } else {
            self.auto_cycle_task.take();
        }
        cx.notify();
    }

    fn start_auto_cycle(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let interval = Duration::from_secs(self.auto_cycle_interval_secs);
        let prompt = self.auto_cycle_prompt.clone();
        let message_editor = self.message_editor.clone();

        self.auto_cycle_task = Some(cx.spawn_in(window, async move |this, cx| {
            loop {
                cx.background_executor().timer(interval).await;

                let should_send = this
                    .update(cx, |this, cx| {
                        this.auto_cycle_enabled
                        ...
                        h_flex()
                            ...
                            .child(
                                h_flex()
                                    ...
                                    .child(self.render_auto_summary_toggle(cx))
                                    .child(self.render_auto_cycle_toggle(cx))
    ...
    fn render_auto_approve_toggle(&self, cx: &Context<Self>) -> impl IntoElement {
        let is_enabled = self.auto_approve_enabled;
        ...
        let tooltip_text = if is_enabled {
            format!(
                "Auto-approve: ON ({}/{})",
                self.auto_approve_count,
                if self.auto_approve_max_count > 0 {
                    self.auto_approve_max_count.to_string()
                } else {
                    "∞".to_string()
                }
            )
        ...
        let is_enabled_for_menu = is_enabled;
        let entity_for_trigger = entity.clone();

        right_click_menu("auto-approve-settings")
            .trigger(move |_, _window, _cx| {
                IconButton::new("auto-approve-toggle", icon)
                    .icon_size(IconSize::Small)
                    .icon_color(if is_enabled_for_menu {
                        Color::Info
                    } else {
                        Color::Muted
                    })
                    ...
                    .on_click({
                        ...
                        move |_, _window, cx| {
                            entity.update(cx, |this, cx| {
                                this.toggle_auto_approve(_window, cx);
                            });
                        }
                    })
                    .into_any_element()
            ...
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, move |menu, _, cx| {
                    ...
                    let current_max = this.auto_approve_max_count;
                    let current_count = this.auto_approve_count;
                    let dec_entity = entity.clone();
                    let inc_entity = entity.clone();
                    let reset_entity = entity.clone();

                    menu.header("Auto-approve Settings")
                        .custom_row(move |_window, _cx| {
                            let dec = dec_entity.clone();
                            let inc = inc_entity.clone();
                            h_flex()
                                .gap_2()
                                .child(
                                    div().child(
                                        IconButton::new("auto-approve-limit-dec", IconName::Dash)
                                            .icon_size(IconSize::Small)
                                            .on_click(move |_, _window, cx| {
                                                dec.update(cx, |this, cx| {
                                                    this.auto_approve_max_count = this
                                                        .auto_approve_max_count
                                                        .saturating_sub(1);
                                                    cx.notify();
                                                });
                                            }),
                                ...
                                .child(
                                    div().min_w(rems(3.)).child(
                                        Label::new(if current_max == 0 {
                                            "∞".into()
                                        } else {
                                            format!("{} 次", current_max)
                                        })
                                        .size(LabelSize::Small),
                                    ),
                                )
                                .child(
                                    div().child(
                                        IconButton::new("auto-approve-limit-inc", IconName::Plus)
                                            .icon_size(IconSize::Small)
                                            .on_click(move |_, _window, cx| {
                                                inc.update(cx, |this, cx| {
                                                    this.auto_approve_max_count = this
                                                        .auto_approve_max_count
                                                        .saturating_add(1);
                                                    cx.notify();
                        ...
                        .item(ContextMenuEntry::new(if current_max == 0 {
                            "已用: 无限".to_string()
                        } else {
                            format!("已用: {} / {}", current_count, current_max)
                        }))
                        .separator()
                        .item(ContextMenuEntry::new("重置计数器").handler(move |_, cx| {
                            reset_entity.update(cx, |this, cx| {
                                this.auto_approve_count = 0;
                                cx.notify();
                            });
                        }))
                })
    ...
    fn render_auto_summary_toggle(&self, cx: &Context<Self>) -> impl IntoElement {
        let is_enabled = self.auto_summary_enabled;
        let icon = if is_enabled {
            IconName::ThreadFromSummary
        } else {
            IconName::NewThread
        };
        let threshold_display = if self.auto_summary_threshold_tokens == u64::MAX {
            "auto".to_string()
        } else {
            crate::humanize_token_count(self.auto_summary_threshold_tokens)
        };
        let tooltip_text = if is_enabled {
            format!("Auto-summary: ON (threshold: {})", threshold_display)
        } else {
            "Auto-summary: OFF".to_string()
        };
        let tooltip_text: SharedString = tooltip_text.into();
        let entity = cx.entity();
        let is_enabled_for_menu = is_enabled;
        ...
        right_click_menu("auto-summary-settings")
            .trigger(move |_, _window, _cx| {
                IconButton::new("auto-summary-toggle", icon)
                    .icon_size(IconSize::Small)
                    .icon_color(if is_enabled_for_menu {
                        Color::Info
                    } else {
                    ...
                    .on_click({
                        ...
                        move |_, _w, cx| {
                            entity.update(cx, |this, cx| {
                                this.toggle_auto_summary(_w, cx);
                            });
                        }
                    })
                    .into_any_element()
            ...
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, move |menu, _, cx| {
                    let this = entity.read(cx);
                    let current_threshold = this.auto_summary_threshold_tokens;
                    let dec_entity = entity.clone();
                    let inc_entity = entity.clone();
                    let k100_entity = entity.clone();
                    let k200_entity = entity.clone();
                    let m1_entity = entity.clone();
                    let m2_entity = entity.clone();

                    menu.header("Auto-summary Settings")
                        .custom_row(move |_window, _cx| {
                            let dec = dec_entity.clone();
                            let inc = inc_entity.clone();
                            h_flex()
                                ...
                                .child(
                                    div().child(
                                        IconButton::new("threshold-dec", IconName::Dash)
                                            .icon_size(IconSize::Small)
                                            .on_click(move |_, _window, cx| {
                                                dec.update(cx, |this, cx| {
                                                    let step = if this.auto_summary_threshold_tokens
                                                        > 100000
                                                    {
                                                        100000
                                                    } else {
                                                    ...
                                                    this.auto_summary_threshold_tokens = this
                                                        .auto_summary_threshold_tokens
                                                        .saturating_sub(step)
                                                        .max(1000);
                                                    cx.notify();
                                                });
                                ...
                                .child(
                                    div().min_w(rems(4.)).child(
                                        Label::new(crate::humanize_token_count(current_threshold))
                                            .size(LabelSize::Small),
                                    ),
                                )
                                .child(
                                    div().child(
                                        IconButton::new("threshold-inc", IconName::Plus)
                                            .icon_size(IconSize::Small)
                                            .on_click(move |_, _window, cx| {
                                                inc.update(cx, |this, cx| {
                                                    let step = if this.auto_summary_threshold_tokens
                        ...
                        .separator()
                        .item(
                            ContextMenuEntry::new("100K (小模型)")
                                .toggleable(IconPosition::Start, current_threshold == 100000)
                                .handler(move |_, cx| {
                                    k100_entity.update(cx, |this, cx| {
                                        this.auto_summary_threshold_tokens = 100000;
                                        cx.notify();
                                    });
                                }),
                        )
                        .item(
                            ContextMenuEntry::new("200K (常见模型)")
                                .toggleable(IconPosition::Start, current_threshold == 200000)
                                .handler(move |_, cx| {
                                    k200_entity.update(cx, |this, cx| {
                                        this.auto_summary_threshold_tokens = 200000;
                                        cx.notify();
                                    });
                                }),
                        )
                        .item(
                            ContextMenuEntry::new("1M (大模型)")
                                .toggleable(IconPosition::Start, current_threshold == 1000000)
                                .handler(move |_, cx| {
                                    m1_entity.update(cx, |this, cx| {
                                        this.auto_summary_threshold_tokens = 1000000;
                                        cx.notify();
                                    });
                                }),
                        )
                        .item(
                            ContextMenuEntry::new("2M (超大模型)")
                                .toggleable(IconPosition::Start, current_threshold == 2000000)
                                .handler(move |_, cx| {
                                    m2_entity.update(cx, |this, cx| {
                                        this.auto_summary_threshold_tokens = 2000000;
                                        cx.notify();
                                    });
                                }),
                        )
    ...
    fn render_auto_cycle_toggle(&self, cx: &Context<Self>) -> impl IntoElement {
        let is_enabled = self.auto_cycle_enabled;
        let icon = if is_enabled {
            IconName::RotateCw
        } else {
            IconName::RotateCcw
        ...
        let tooltip_text = if is_enabled {
            format!(
                "Auto-cycle: ON (every {}s, prompt: \"{}\")",
                self.auto_cycle_interval_secs, self.auto_cycle_prompt
            )
        } else {
        ...
        right_click_menu("auto-cycle-settings")
            .trigger(move |_, _window, _cx| {
                IconButton::new("auto-cycle-toggle", icon)
                    .icon_size(IconSize::Small)
                    .icon_color(if is_enabled_for_menu {
                        Color::Info
                    } else {
            ...
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, move |menu, _, cx| {
                    let this = entity.read(cx);
                    let current_interval = this.auto_cycle_interval_secs;
                    let current_prompt = this.auto_cycle_prompt.clone();
                    let dec_entity = entity.clone();
                    let inc_entity = entity.clone();
                    let prompt_continue = entity.clone();
                    let prompt_improve = entity.clone();
                    let prompt_modify = entity.clone();
                    let current_prompt_clone = current_prompt.clone();

                    menu.header("Auto-cycle Settings")
                        .custom_row(move |_window, _cx| {
                            let dec = dec_entity.clone();
                            let inc = inc_entity.clone();
                            h_flex()
                                ...
                                .child(
                                    div().child(
                                        IconButton::new("interval-dec", IconName::Dash)
                                            .icon_size(IconSize::Small)
                                            .on_click(move |_, _window, cx| {
                                                dec.update(cx, |this, cx| {
                                                    this.auto_cycle_interval_secs = this
                                ...
                                .child(
                                    div().min_w(rems(2.)).child(
                                        Label::new(format!("{}s", current_interval))
                                            .size(LabelSize::Small),
                                    ),
                                )
                                .child(
                                    div().child(
                                        IconButton::new("interval-inc", IconName::Plus)
                                            .icon_size(IconSize::Small)
                                            .on_click(move |_, _window, cx| {
                                                inc.update(cx, |this, cx| {
                                                    this.auto_cycle_interval_secs = this
                        ...
                        .custom_row(move |_window, _cx| {
                            h_flex()
                                ...
                                .child(
                                    Label::new(format!("\"{}\"", current_prompt))
                                        .size(LabelSize::Small),
                                )
                                .into_any_element()
                        })
                        .separator()
                        .item(
                            ContextMenuEntry::new("提示词: continue")
                                .toggleable(IconPosition::Start, current_prompt_clone == "continue")
                                .handler(move |_, cx| {
                                    prompt_continue.update(cx, |this, cx| {
                                        this.auto_cycle_prompt = "continue".into();
                                        cx.notify();
                                    });
                                }),
                        )
                        .item(
                            ContextMenuEntry::new("提示词: 自主寻找改进点")
                                .toggleable(
                                    IconPosition::Start,
                                    current_prompt_clone == "自主寻找改进点",
                                )
                                .handler(move |_, cx| {
                                    prompt_improve.update(cx, |this, cx| {
                                        this.auto_cycle_prompt = "自主寻找改进点".into();
                                        cx.notify();
                                    });
                                }),
                        )
                        .item(
                            ContextMenuEntry::new("提示词: 继续修改")
                                .toggleable(IconPosition::Start, current_prompt_clone == "继续修改")
                                .handler(move |_, cx| {
                                    prompt_modify.update(cx, |this, cx| {
                                        this.auto_cycle_prompt = "继续修改".into();
                                        cx.notify();
                                    });
                                }),
                        )
...
impl Render for ThreadView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .on_action(cx.listener(|this, _: &ToggleAutoApprove, window, cx| {
                this.toggle_auto_approve(window, cx);
            }))
