...
use crate::{
    ...
    NewNativeAgentThreadFromSummary, NewThread, OpenAddContextMenu, OpenAgentDiff, RejectAll,
    RejectOnce, RemoveFirstQueuedMessage, ScrollOutputLineDown, ScrollOutputLineUp,
    ...
    ToggleAutoApprove, ToggleFastMode, ToggleProfileSelector, ToggleThinkingEffortMenu,
...
/// Pending auto-summary state to inherit when creating a new root thread.
#[derive(Clone)]
struct AutoSummaryPending {
    /// The prompt to auto-send in the new thread.
    prompt: SharedString,
    /// Whether auto-approve should be enabled on the new thread.
    auto_approve_enabled: bool,
    /// Maximum auto-approvals for the new thread.
    auto_approve_max_count: u32,
    /// Whether auto-cycle should be enabled on the new thread.
    auto_cycle_enabled: bool,
    /// Auto-cycle prompt.
    auto_cycle_prompt: SharedString,
    /// Auto-cycle interval in seconds.
    auto_cycle_interval_secs: u64,
    /// Whether auto-summary should be enabled on the new thread.
    auto_summary_enabled: bool,
    /// Auto-summary threshold tokens.
    auto_summary_threshold_tokens: u64,
}

/// Bridge for passing auto-summary inherited settings across ConversationView
/// instances. `handle_thread_event` (on the OLD ConversationView) writes here,
/// and `new_thread_view` (on the NEW ConversationView) reads from here.
fn auto_summary_bridge() -> &'static Mutex<Option<AutoSummaryPending>> {
    static BRIDGE: OnceLock<Mutex<Option<AutoSummaryPending>>> = OnceLock::new();
...
pub struct ConversationView {
    ...
    /// When set after auto-summary, the next ThreadView created as root will
    /// inherit these settings and auto-send the prompt.
    auto_summary_pending: RefCell<Option<AutoSummaryPending>>,
    /// Subagent reports queued for delivery to the parent thread.
    /// Keyed by parent session_id, values are report texts.
    pending_subagent_reports: RefCell<HashMap<acp::SessionId, Vec<String>>>,
...
impl ConversationView {
    pub fn new(
        ...
        Self {
            ...
            auto_summary_pending: RefCell::new(None),
            pending_subagent_reports: RefCell::new(HashMap::default()),
    ...
    fn new_thread_view(
        ...
        // Inherit auto-summary settings from the parent ConversationView.
        // The parent writes them to the global bridge in handle_thread_event
        // (Stopped → auto-summary branch) before dispatching the action.
        if self.auto_summary_pending.borrow().is_none() {
            if let Some(AgentInitialContent::ThreadSummary { .. }) = initial_content.as_ref() {
                let inherited = auto_summary_bridge()
                    .lock()
                    .unwrap_or_else(|e| e.into_inner())
                    .take();
                let pending = inherited.unwrap_or_else(|| AutoSummaryPending {
                    prompt: "continue".into(),
                    auto_approve_enabled: false,
                    auto_approve_max_count: 0,
                    auto_cycle_enabled: false,
                    auto_cycle_prompt: "continue".into(),
                    auto_cycle_interval_secs: 30,
                    auto_summary_enabled: false,
                    auto_summary_threshold_tokens: u64::MAX,
                });
                self.auto_summary_pending.replace(Some(pending));
            }
        }
        ...
        // If there's a pending auto-summary, inherit the settings.
        // Defer the auto-send to the next event loop iteration so that
        // the new ThreadView is fully settled before we call send().
        if let Some(pending) = self.auto_summary_pending.take() {
            view.update(cx, |view, _cx| {
                view.auto_approve_enabled = pending.auto_approve_enabled;
                view.auto_approve_max_count = pending.auto_approve_max_count;
                view.auto_cycle_enabled = pending.auto_cycle_enabled;
                view.auto_cycle_prompt = pending.auto_cycle_prompt;
                view.auto_cycle_interval_secs = pending.auto_cycle_interval_secs;
                view.auto_summary_enabled = pending.auto_summary_enabled;
                view.auto_summary_threshold_tokens = pending.auto_summary_threshold_tokens;
            });
            // Use window.defer to delay the send until the next frame,
            // after the new ThreadView is fully initialized and event
            // subscriptions are active.
            //
            // NOTE: When initial_content is ThreadSummary, the editor already
            // contains a @thread-id mention via insert_thread_summary in
            // ThreadView::new. Do NOT clear and overwrite it — send() will
            // resolve the mention asynchronously (waiting for the LLM summary)
            // and include it as context. Clearing it here would discard the
            // summary context and send only "continue".
            window.defer(cx, {
                let view = view.clone();
                move |window, cx| {
    ...
    /// Flush queued subagent reports to the target thread.
    /// Sends all pending reports as a single combined message.
    fn flush_subagent_reports(
        pending: &RefCell<HashMap<acp::SessionId, Vec<String>>>,
        target_session_id: &acp::SessionId,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let reports = pending.borrow_mut().remove(target_session_id);
        ...
        let this = cx.entity();
        let Some(parent_thread) = this.read(cx).thread_view(target_session_id) else {
            // Thread not loaded yet; put reports back.
            pending
                .borrow_mut()
                .insert(target_session_id.clone(), reports);
            return;
        };
        // Only flush if parent is idle (non-preemptive).
        if parent_thread.read(cx).thread.read(cx).status() != ThreadStatus::Idle {
            pending
                .borrow_mut()
                .insert(target_session_id.clone(), reports);
            return;
        }
        let combined = if reports.len() == 1 {
            format!(
                "[Subagent Report]\n\n{}",
                reports.into_iter().next().unwrap()
            )
        } else {
            let mut body = String::from("[Subagent Reports]\n");
            for (i, r) in reports.iter().enumerate() {
                body.push_str(&format!("\n--- Report {} ---\n{}\n", i + 1, r));
            }
            body
        };
        let editor = parent_thread.read(cx).message_editor.clone();
        parent_thread.update(cx, |parent, cx| {
            editor.update(cx, |editor, cx| {
                editor.clear(window, cx);
                editor.insert_text(&combined, window, cx);
            });
            parent.send(window, cx);
        });
    }
    ...
    fn handle_thread_event(
        ...
        match event {
            ...
            AcpThreadEvent::ToolAuthorizationRequested(_) => {
                ...
                // Auto-approve: if the active thread has auto-approve enabled,
                // automatically approve the pending tool call with AllowOnce.
                let should_auto_approve = self
                    .active_thread()
                    .as_ref()
                    .map(|a| {
                        let enabled = a.read(cx).auto_approve_enabled;
                        let max_count = a.read(cx).auto_approve_max_count;
                        let count = a.read(cx).auto_approve_count;
                        enabled && (max_count == 0 || count < max_count)
                    })
                    .unwrap_or(false);

                if should_auto_approve {
                    let pending = self.pending_tool_call(cx);
                    let authorize_data =
                        pending
                            .as_ref()
                            .and_then(|(auth_session_id, tool_call_id, options)| {
                                options
                    ...
                    if let Some((auth_session_id, tool_call_id, option_id)) = authorize_data {
            ...
            AcpThreadEvent::Stopped(stop_reason) => {
                ...
                if is_subagent {
                    if *stop_reason == acp::StopReason::EndTurn {
                        ...
                        // Auto-report subagent output to the parent thread.
                        // Collect the subagent's last assistant message as the report.
                        let report = thread.read(cx).entries().iter().rev().find_map(|entry| {
                            if let AgentThreadEntry::AssistantMessage(msg) = entry {
                                if msg.is_subagent_output {
                                    let chunks = msg
                                        .chunks
                                        .iter()
                                        .filter_map(|c| match c {
                                            ...
                                            | AssistantMessageChunk::Thought { block } => {
                                                match block {
                                                    acp_thread::ContentBlock::Markdown {
                                                        markdown,
                                                    } => {
                                                        Some(markdown.read(cx).source().to_string())
                                                    }
                                                    acp_thread::ContentBlock::ResourceLink {
                                                        resource_link,
                                                    } => Some(resource_link.uri.clone()),
                                                    acp_thread::ContentBlock::Image { .. } => {
                        ...
                        // Queue the report for delivery to the parent thread.
                        if let Some(report_text) = report {
                            let parent_session_id = thread.read(cx).parent_session_id().cloned();
                            if let Some(parent_id) = parent_session_id {
                                // Route to current root thread if parent was summarized.
                                let target_id = self
                                    .root_session_id
                                    .as_ref()
                                    .filter(|root_id| **root_id != parent_id)
                                    .cloned()
                                    .unwrap_or(parent_id);
                                self.pending_subagent_reports
                                    .borrow_mut()
                                    .entry(target_id.clone())
                                    .or_default()
                                    .push(report_text);
                                // Try to flush queued reports to the parent.
                                Self::flush_subagent_reports(
                                    &self.pending_subagent_reports,
                                    &target_id,
                                    window,
                                    cx,
                ...
                // Flush any queued subagent reports to the root thread now that it's idle.
                if let Some(root_id) = self.root_session_id.clone() {
                    Self::flush_subagent_reports(
                        &self.pending_subagent_reports,
                        &root_id,
                        window,
                ...
                // Auto-summarization: if the thread needs summarization due to token limit,
                // automatically start a new thread from a summary, inheriting the current
                // auto-settings.
                let did_auto_summary = if let Some(active) = self.root_thread_view() {
                    let needs_summary = active.read(cx).needs_auto_summary;
                    if needs_summary && !is_subagent {
                        let session_id = active.read(cx).session_id.clone();
                        // Save all auto-settings to inherit in the new thread.
                        let pending = {
                            let a = active.read(cx);
                            let prompt = if a.auto_cycle_enabled {
                                a.auto_cycle_prompt.clone()
                            } else {
                                "continue".into()
                            };
                            Some(AutoSummaryPending {
                                prompt,
                                auto_approve_enabled: a.auto_approve_enabled,
                                auto_approve_max_count: a.auto_approve_max_count,
                                auto_cycle_enabled: a.auto_cycle_enabled,
                                auto_cycle_prompt: a.auto_cycle_prompt.clone(),
                                auto_cycle_interval_secs: a.auto_cycle_interval_secs,
                                auto_summary_enabled: a.auto_summary_enabled,
                                auto_summary_threshold_tokens: a.auto_summary_threshold_tokens,
                            })
                        };
                        active.update(cx, |active, _cx| {
                            active.needs_auto_summary = false;
                        });
                        // Store the inherited state via the global bridge so that
                        // the NEW ConversationView (created by dispatch_action) can
                        // read it in new_thread_view.
                        *auto_summary_bridge()
                            .lock()
                            .unwrap_or_else(|e| e.into_inner()) = pending.clone();
                ...
                // If this Stopped event is for a thread with a pending_continue_prompt,
                // send the prompt now (used by auto-summary to continue in new thread).
                if let Some(active) = self.thread_view(&session_id) {
                    let prompt = active.read(cx).pending_continue_prompt.clone();
                    if let Some(prompt_text) = prompt {
                        active.update(cx, |active, cx| {
                            active.pending_continue_prompt = None;
                            let editor = active.message_editor.clone();
                            editor.update(cx, |editor, cx| {
                                editor.clear(window, cx);
                                editor.insert_text(&prompt_text, window, cx);
                            });
                            active.send(window, cx);
                        });
                    }
                }

                // Auto-cycle heartbeat: if auto-cycle is enabled on the root thread,
                // the editor is empty, and the thread is idle, trigger the heartbeat now.
                // Skip if we just triggered auto-summary (new thread will have its own cycle).
                if !did_auto_summary {
                    if let Some(active) = self.root_thread_view() {
                        let should_auto_cycle = active.read(cx).auto_cycle_enabled
                            && active.read(cx).thread.read(cx).status() == ThreadStatus::Idle
                            && active.read(cx).message_editor.read(cx).is_empty(cx);
                        if should_auto_cycle && !should_send_queued {
                            let prompt = active.read(cx).auto_cycle_prompt.clone();
                            let message_editor = active.read(cx).message_editor.clone();
                            active.update(cx, |active, cx| {
            ...
            AcpThreadEvent::TokenUsageUpdated => {
                if let Some(active) = self.thread_view(&session_id) {
                    active.update(cx, |active, cx| {
                        ...
                        // Auto-summarization: if auto-summary is enabled and token usage
                        // reaches the configured threshold, mark that we need a summary
                        // on the next Stopped event.
                        if active.auto_summary_enabled && !active.needs_auto_summary {
                            if let Some(token_usage) = active.thread.read(cx).token_usage() {
                                let threshold = active.auto_summary_threshold_tokens;
                                let used = token_usage.used_tokens;
                                if threshold != u64::MAX && used >= threshold {
                                    active.needs_auto_summary = true;
                                }
                            }
                        }
                    });
