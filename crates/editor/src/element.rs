...
use gpui::{
    ...
    MousePressureEvent, MouseUpEvent, ObjectFit, PaintQuad, ParentElement, Pixels, PressureStage,
    ...
    WeakEntity, Window, anchored, deferred, div, fill, img, linear_color_stop, linear_gradient,
...
impl EditorElement {
    ...
    fn paint_background(&self, layout: &mut EditorLayout, window: &mut Window, cx: &mut App) {
        window.paint_layer(layout.hitbox.bounds, |window| {
            ...
            if let Some(background_image) = layout.background_image.as_mut() {
                let bounds = layout.position_map.text_hitbox.bounds;
                window.with_content_mask(Some(ContentMask { bounds }), |window| {
                    background_image.paint(window, cx);
                });
            }
        ...
        window.with_rem_size(rem_size, |window| {
            window.with_text_style(Some(text_style), |window| {
                window.with_content_mask(Some(ContentMask { bounds }), |window| {
                    ...
                    {
                        let (background_image_path, background_image_opacity) = {
                            let settings = EditorSettings::get_global(cx);
                            (
                                settings
                                    .editor_background_image
                                    .as_ref()
                                    .and_then(|path| path.as_path().is_file().then_some(path))
                                    .cloned(),
                                settings
                                    .editor_background_opacity
                                    .unwrap_or(1.0)
                                    .clamp(0.0, 1.0),
                            )
                        };
                        ...
                        if let Some(path) = background_image_path {
                            let available_width = text_hitbox.size.width;
                            let available_height = text_hitbox.size.height;
                            if available_width > Pixels::ZERO && available_height > Pixels::ZERO {
...
pub struct EditorLayout {
    ...
    gutter_hitbox: Hitbox,
    content_origin: gpui::Point<Pixels>,
    background_image: Option<AnyElement>,
